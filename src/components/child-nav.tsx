  ...
  return (
    <header className="sticky top-0 z-20 border-b border-[#2a2118]/10 bg-[#fff8ec]/90 backdrop-blur-md">
      <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-4 py-3">
        <Link to="/" className="flex items-center gap-3 text-[#2a2118] no-underline">
        ...
        <nav className="flex flex-wrap gap-2">
          {links.map((link) => (
            <Link
              ...
              activeProps={{ className: 'rounded-full bg-[#2a2118] px-3 py-1.5 text-sm font-bold text-[#fff8ec] no-underline' }}
            >
              {link.label}
            </Link>
          ))}
